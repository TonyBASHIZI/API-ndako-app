...
class Save{
	public function saveRetrait(){
		if (isset($_POST)) {
            ...
            {
                $checkPWD=mysqli_query(Constants::connect(), "select * from detail_carte where ref_nfc='".$matricule_client."' and mot_de_passe='".$clientPwd."'");
                    ...
                    if(mysqli_num_rows($checkAgent)==1)
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                $querry3="update detail_carte  set montant=montant-'".$montant_retrait."' where ref_nfc='".$matricule_client."';";
                                $res=mysqli_multi_query(Constants::connect(),$querry2.$querry3);
                                // $res=mysqli_multi_query(Constants::connect(), $querry1.$querry2.$querry3);
                                if($res)
                                {
                                    $msg["success"]= "success";
        ...
        if (isset($_POST)) {
            ...
            else 
            {
                ...
                else
                {
                    try 
                    {
                        $querry5="INSERT INTO `t_client` (`matr_client`, `nom`, `postnom`, `prenom`, `adresse`, `reseaux`, `tel`, `refcat`, `id_carte`, sexe, etatcivil, age, affiliation) VALUES('$matricule','$nom', '$postnom', '$prenom', '$adresse', 'M00-M-4219-17', '$telephone', '01', '$ref_IDcarte', '$sexe', '$etatcivil', '$age', '$affiliation');";
...
// else if(strtolower($_POST['transaction'])=="remise"){
// 	$saveInstance->saveRemise();
// }
?>